...
One approximation is carried throughout and is the same one the load
...
is not $\overline{f(\gamma)}$. The two differ by an amount this work has
not bounded. What removes the earlier objection is not that the
approximation is absent but that the quantity is a location read off one
solve rather than a small difference between two large and nearly equal
cycle means.
