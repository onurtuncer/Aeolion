...
conditions are computed. Recomputing $\alpha = 20^\circ$, $T_c = 0.5$
after arriving from $16^\circ$ rather than $18^\circ$ moves the
span-mean separation point from $0.719$ to $0.706$: both solutions sit in
limit cycles of the same length, and their \emph{means} differ by $0.013$
against a fan effect of $0.018$ at that same condition. This does not
disturb the increments reported here, which difference powered against
power-off at a common attitude with both solves having traversed the same
grid; but it does mean the absolute separation locations carry a path
dependence of the same order as the effect measured against them, and
whether that dependence cancels in the difference is untested. It also
qualifies the earlier finding that limit-cycle means are iteration-path
independent: that result holds at fixed continuation, and warm-start
history evidently matters a good deal more than the relaxation does.
