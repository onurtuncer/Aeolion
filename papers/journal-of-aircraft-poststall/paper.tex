...
the unsteady cross-check: two-dimensional discrete-vortex sections with
...
\begin{figure}[tb]
  ...
  \caption{The anchored section model against the Sheldahl--Klimas
    ...
    cross-check of Section~\ref{sec:cross-check}, mean $\pm$ RMS --- read for
...
\subsection{The unsteady cross-check}
\label{sec:cross-check}

\begin{figure}[tb]
  \centering
  \includegraphics[width=\linewidth]{figures/crosscheck-sketch.pdf}
  \caption{The unsteady cross-check, schematically. Left, the
    two-dimensional tier: a Glauert bound sheet on the chord,
    trailing-edge vortices shed every step with Kelvin's theorem closing
    their strengths, leading-edge vortices while the leading-edge
    suction parameter exceeds its critical value. Right, the
    three-dimensional tier: one ring per strip, the one-step buffer ring
    whose near segment cancels the ring's trailing-edge closer, the
    wake carried by vector particles (merged conversion, transpose-scheme
    stretching, direct $N^2$ induction), and the counter-rotating
    leading-edge flux pair on separated strips.}
  \label{fig:crosscheck-sketch}
\end{figure}
...
section in the manner of Ramesh et al.~\cite{ramesh2014lesp}. The
plate's bound vorticity is the Glauert series
\begin{equation}
  \gamma(\theta,t) = 2U\!\left[A_0(t)\,\frac{1+\cos\theta}{\sin\theta}
    + \sum_{n\ge 1} A_n(t)\sin n\theta\right],
\end{equation}
whose coefficients are projected each step from the downwash $w$ of
everything that is not the bound sheet (freestream and every free
vortex): $A_0 = \tfrac{1}{\pi}\int_0^\pi (w/U)\,d\theta$,
$A_n = -\tfrac{2}{\pi}\int_0^\pi (w/U)\cos n\theta\,d\theta$. One
trailing-edge vortex is shed per step with its strength closing
Kelvin's theorem,
$\Gamma_b + \sum_{wake}\Gamma = 0$ with
$\Gamma_b = \pi c U (A_0 + A_1/2)$; while $|A_0|$ -- the leading-edge
suction parameter -- exceeds its critical value, a leading-edge vortex
is shed as well, the pair solving Kelvin and $|A_0| = \mathrm{LESP}_{cr}$
as a $2\times2$ linear system. Loads come from the impulse of the
tracked vorticity,
\begin{equation}
  \mathbf{F} = \rho\,\frac{d}{dt}\sum_k \Gamma_k\,(z_k,\,-x_k),
  \label{eq:impulse2d}
\end{equation}
exact at any incidence where the linearized force formulas are not.
The tier's own calibration is pinned in the test suite: the impulsive
start follows Wagner's function, the attached mean recovers
...
vortex ring per strip with flow tangency at the three-quarter-chord
points, a one-step buffer ring carrying $\Gamma_i(t{-}1)$ whose near
segment cancels the bound ring's trailing-edge closer, and a wake of
vector particles $(\mathbf{x}_p, \boldsymbol{\alpha}_p)$ with the
regularized induction
\begin{equation}
...
\end{equation}
Particles convect with the full local velocity and stretch in the
transpose scheme,
$d\boldsymbol{\alpha}_p/dt = (\nabla\mathbf{u})^{T}\boldsymbol{\alpha}_p$,
whose exact conservation of total vector strength is what keeps the
three-dimensional impulse ledger,
\begin{equation}
  \mathbf{F} = -\rho\,\frac{d}{dt}\Bigl[
    \sum_i \Gamma_i \mathbf{S}_i
    + \sum_i \Gamma_i^{buf}\mathbf{S}_i^{buf}
    + \tfrac{1}{2}\sum_p \mathbf{x}_p\times\boldsymbol{\alpha}_p
  \Bigr],
  \label{eq:impulse3d}
\end{equation}
readable over a chaotically shedding wake ($\mathbf{S}$: the loops'
vector areas). Strips past their separation boundary -- the same
$f(\eta,\alpha)$ the anchored model consumes -- shed the classical
shear-layer flux $d\Gamma/dt = \tfrac{1}{2}V_{loc}^2$ per unit span
from the leading edge, as a counter-rotating pair that leaves the
total circulation untouched. The full formulation, including the
merged wake-conversion that makes Eq.~\eqref{eq:impulse3d} usable and
the measured failure modes of its alternatives, is documented with the
implementation.

Its cross-check products appear as the mean-$\pm$-RMS band in
Fig.~\ref{fig:validation}. Three readings matter. First, the
\emph{break}: the cross-check leaves the attached line between $10^\circ$
...
is a mean over a permanently shedding street. Third, the cross-check's own
...
two-dimensional cross-check is therefore read for its break location and
its fluctuation content, not its mean levels --- and its overshoot is
the standing, now-quantified argument for the three-dimensional tier.

That tier is implemented and pinned by its own suite: attached, the
impulse and circulation readings of the lift agree to $0.2\%$ and
follow Wagner's growth; Kelvin-consistent shedding holds strip by strip
including across planform gaps; and on an $AR = 6$ plate normal to the
stream the three-dimensional street --- stretching enabled --- carries
a mean $C_N$ of $0.4$--$0.9$ across leading-edge flux-model variants at
test-budget averaging, where its two-dimensional sibling carries $3.3$:
spanwise breakup collapses the street toward the scale of the
finite-plate ceiling (1.22), which is the tier's reason to exist,
demonstrated --- though the mean's precise level is not a converged
number. At configuration level the declared attitude runs
($\alpha = 30/60/90^\circ$, $\beta = 0/15^\circ$, full-span lattice)
returned a MEASURED statistical verdict rather than a table: the
fluctuation-to-mean ratio of the inviscid particle street runs 3 to 40
across the attitudes and grows with incidence and with the averaging
window, so mean loads do not converge within any feasible average at
this tier's fidelity --- converging them to ten percent would demand
hundreds of independent shedding periods together with the viscous
core spreading and spanwise resolution of a production vortex-particle
method, which is beyond a cross-check's charter and outside this
paper's claims. The tier's deliverables therefore stand as stated: the
attached-flow exactness checks, the fluctuation content the
quasi-steady map cannot see (which needs no converged mean), and the
structural deep-stall result that spanwise breakup collapses the
street from the two-dimensional level toward the finite-plate scale.
A mean-capable unsteady comparison is identified, quantified work ---
not pending work disguised as a formality. A regularized variant was
built and piloted to sharpen that quantification: eddy-viscosity core
spreading with like-signed far-wake merging, second-order convection,
and vorticity-consistent relaxation bounds the short-window street
(the normal plate's force RMS falls two orders) and leaves the
attached checks untouched, but at $\alpha = 60^\circ$ over sixty
convective times the configuration street remains
fluctuation-dominated: the coarse lattice's co-rotating consolidation
feeds structures faster than the model dissipates them, and the
batch-mean confidence intervals stay wider than the means. Nor is modest resolution the missing ingredient: with an
octree-accelerated evaluation (pinned exactly equivalent to direct
summation when fully opened) buying a doubling of spanwise and
temporal resolution, the same attitude grew categorically worse ---
smaller cores sharpen the close-range consolidation faster than the
added resolution resolves the cascade. Converged configuration means
at these attitudes belong to the overlap-resolved vortex-particle
regime --- several particles per shed structure, million-particle
wakes, subgrid dissipation, and fast-multipole or GPU summation --- a
solver project in its own right, delimited here by measurement rather
than promised. Two rejected shortcuts (a merge rule that annihilated
counter-rotating pairs, and impulse bookkeeping that hid systematic
strength edits) each produced tightly converged means that were wrong
by an order of magnitude, and are recorded as warnings where they
were found.
