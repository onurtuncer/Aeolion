...
reverses the sign of the measured delay. Most conditions are insensitive,
and the finer continuation is on general grounds the better
approximation, so the shipped tables are the better-resolved of the two
compared --- but they are not demonstrated to be grid-converged, and that
check is outstanding. This affects the \varid{aero*} and
...
incidence.
\item Repeat the post-stall sweeps on a finer incidence grid and confirm
that the increments agree, settling whether the continuation-path
dependence cancels in a powered-minus-power-off difference. The separation
points themselves are now measured directly rather than inferred.
