...
file's own verification data can confirm.

The cost of one file is coarser regeneration granularity, and it is
avoided by construction: each sweep exports its own JSON, and a
...
generating program passed the contract's moment reference point into the solver
...
silently. The numeric meaning of each sign is instead fixed by a
...
tables. The viscous-coupled solve accumulates the circulatory
...
returns $-0.452$, and that value is now confirmed both by an automated check and by
...
receives the positive angle---are a convention, and the file fixes them by
...
Source: the viscous-coupled solve---vortex lattice with source-panelled
...
semi-infinite vortex-cylinder induction model.}
\label{tab:cpltables} \\
\toprule
\textbf{varID} & \textbf{Axes} & \textbf{Quantity} & \textbf{Notes} \\
\midrule
...
solve can report the powered and power-off separation locations and
...
\input{tables/propulsion-baseline}
...
single-row lattice, and through the viscous-coupled path that actually
...
through a tested body-axis extractor whose conversions are verified automatically.
...
\paragraph{It was validated, not asserted.} The same computation was re-run
...
the daggered entries are not steady states. And the increment changes
sign more than once past stall---positive near $40$--$45\dg$, negative
through $50$--$80\dg$, positive again at $90\dg$---at magnitudes of a few
thousandths against an attached value near $0.0096$.

That wobble was initially suspected to be cycle-mean scatter. It is not.
Re-running the whole sweep on an independent iteration path---damped
relaxation of $0.02$ against $0.05$, both running to the iteration
ceiling as cycle means---reproduces every value to within $0.1\%$, and
the sign at every attitude. The cycle means are iteration-path
independent here exactly as they are for the baseline map, so the sign
changes are a determinate property of the model rather than noise.

What that does \emph{not} settle is whether the aircraft behaves that
way. Past stall the model rests on the anchored section model, whose
$C_{L\max}$ is itself an upper bound, so a reproducible sign is a
statement about the tabulation and not yet about the vehicle.
...
looking entirely reasonable. An automated check confirms $\tau$ at its
...
and quarter-chord values, and confirms that a deflection produces a rolling
...
additional sweep of the program that already existed.
...
verified against the textbook figure for an
$A\!R = 6$ wing.
\item \textbf{Vane modes.} Pure pitch, pure yaw and pure roll commands,
each confirming the dominant components and recording the off-axis
...
Section~\ref{sec:buildup} to dimensional forces and moments, confirming
...
\noindent
Each generating program exports JSON independently and the assembler consumes the
...
The attribution to delayed separation is no longer an inference: the
separation point is now read directly off the coupled solve at both
power settings, and the delay is present, thrust-ordered at every
attitude to $70\dg$, and peaks at $+0.065$ chord near $30\dg$. That
measurement also removed a false signature --- the threefold jump in the
increment between $14\dg$ and $16\dg$ is \emph{not} the separation delay,
which moves by only a factor of $1.09$ across the same step; the jump
marks the onset of limit-cycle behaviour. The interaction model is also
...
shift is thin-airfoil theory's, and so is the accompanying section
pitching-moment increment, which the same theory supplies and the model
now carries. What remains unmodelled is gap leakage and the viscous decay
of effectiveness at large deflection---both of which reduce real control
...
\item \textbf{The post-stall sweeps are continuations, and the path
is part of the condition.} Every solve above the stall is warm-started
from the preceding incidence, so which attitudes were visited helps
determine which limit cycle a condition settles into --- it is not merely
the order in which the tables were filled. Measured: arriving at
$\alpha = 20\dg$, $T_c = 0.5$ from $16\dg$ rather than $18\dg$ moves the
span-mean separation point by $0.013$, against a fan effect of $0.018$ at
that same condition, and at $\alpha = 26\dg$, $T_c = 1$ a coarser path
reverses the sign of the measured delay.

That check has since been done, by repeating the sweep at half the
incidence step: $113$ of the $125$ shared conditions agree to better than
$10^{-6}$, so the tables are grid-converged over ninety percent of their
extent. The twelve that do not are isolated bistable conditions, all past
the stall, each a single solve settling into a different limit cycle
according to the attitude it was approached from --- not a drift, and
individually identifiable. Where only the power-off solve moves, its shift
passes through every thrust column undiminished; where both move together
the increment very nearly cancels. Both occur, so cancellation is real but
cannot be assumed. The affected rows carry an uncertainty of order $0.01$
in separation location, $0.027$ at worst. This affects the \varid{aero*} and
\varid{coupling*} post-stall rows, which are generated the same way. It
also narrows the earlier finding that limit-cycle means are
iteration-path independent: that holds at fixed continuation, and
warm-start history matters considerably more than the relaxation does.

\item \textbf{The tables are the ascending-$\alpha$ branch, and that is
adequate.} Every sweep is a continuation started below the stall, so the
tables are one branch of a system that could in principle have two.
Sweeping the same grid downward from $90\dg$ settles what that costs.
Below $14\dg$ the branches agree to $6.6\times10^{-4}$ in $C_Z$ ---
attached flow is path-independent, as it must be. Only seven of
twenty-five attitudes differ by more than $10^{-3}$ at all, and the
largest departures are $1.65\%$ of $C_Z$ at $\alpha = 24\dg$ and $2.6\%$
of $C_m$ at $26\dg$.

This is \emph{not} a classical hysteresis loop, and the distinction
matters for how the model is built. A two-valued system would show the
branches parting across the whole post-stall range and rejoining at its
ends; instead six of the seven cluster in $18$--$35\dg$, the branches
agree at $16\dg$ and $20\dg$ \emph{inside} that band, and agree exactly
from $40$ through $70\dg$. The signature is isolated bistable conditions
rather than two branches --- the same one the incidence-step refinement
produced. The tables therefore need no branch axis; the affected
attitudes need wider uncertainty, and $1.65\%$ sits well inside the
$\pm10$--$20\%$ already carried on the parasite buildup. One departure,
$1.76\%$ of $C_Z$ at $\alpha = 80\dg$, falls outside the stall band and
is recorded without an explanation.

\item \textbf{Quantities evaluated at the cycle mean carry a
nonlinearity error, and it is negligible.} Anything the solver reports
past stall is evaluated on a final sweep at the cycle-mean circulation,
so for a nonlinear $g$, $g(\bar\gamma) \neq \overline{g(\gamma)}$. The
leading term is second order in the cycle width, and the solve carries
the per-strip incidence variance needed to evaluate it: for the
separation point it lands at $10^{-15}$ to $10^{-10}$ against values of
order unity. The cycle is wide in circulation and very nearly stationary
in local incidence. Stated because it is the approximation a reader would
reasonably suspect, and it is not the one that matters --- the preceding
limit is.
...
incidence.
\item Decide whether the twelve grid-sensitive conditions
(Section~\ref{sec:limits}) should be resolved by bisecting the continuation
around them, or declared and carried as an uncertainty. They are named, so
either is tractable; at present they are declared.
