...
\paragraph{One known non-conformance.} S-119 Annex A prescribes standard
variable names for common flight-dynamics quantities. The variable
identifiers in this model (Table~\ref{tab:inputs}) were chosen for
namespacing---the \varid{aero*}, \varid{prop*}, \varid{coupling*}
grouping described below---and have not yet been reconciled against
Annex A. Reconciliation is an open item
(Section~\ref{sec:open}); until it is done, a consumer expecting
Annex A names must map them, and the file's variable descriptions carry
enough information for that mapping to be mechanical.
...
\begin{longtable}{@{}llp{74mm}@{}}
\caption{Input variable definitions.}
\label{tab:inputs} \\
\toprule
\textbf{varID} & \textbf{Units} & \textbf{Meaning} \\
\midrule
\endfirsthead
\multicolumn{3}{@{}l}{\small\itshape Table~\ref{tab:inputs}, continued} \\
\toprule
\textbf{varID} & \textbf{Units} & \textbf{Meaning} \\
\midrule
\endhead
\bottomrule
\endfoot
\varid{alphaDeg} & deg & angle of attack $\alpha$ \\
\varid{betaDeg} & deg & sideslip angle $\beta$ \\
\varid{trueAirspeedMps} & m/s & true airspeed $V_\infty$ \\
\varid{airDensityKgpm3} & kg/m$^3$ & air density $\rho$ \\
\varid{rollRateRadps} & rad/s & body roll rate $p$ \\
\varid{pitchRateRadps} & rad/s & body pitch rate $q$ \\
\varid{yawRateRadps} & rad/s & body yaw rate $r$ \\
\varid{propSpeedRevps} & rev/s & rotor speed $n$ \\
\varid{aileronDeg} & deg & antisymmetric aileron command $\delta_a$,
right surface; the left surface deflects to $-\delta_a$ \\
\varid{vanePitchDeg} & deg & vane pitch mode command $\delta_P$ \\
\varid{vaneYawDeg} & deg & vane yaw mode command $\delta_Y$ \\
\varid{vaneRollDeg} & deg & vane roll mode command $\delta_R$ \\
\end{longtable}
...
increment from baseline. \textbf{BLOCKED}, \\
\varid{aeroDCY} & $\alpha\times\delta_a$ & $\Delta C_Y$ & Section
\ref{sec:aileron}: the generating \\
\varid{aeroDCZ} & $\alpha\times\delta_a$ & $\Delta C_Z$ & path cannot
represent a hinge, \\
\varid{aeroDCl} & $\alpha\times\delta_a$ & $\Delta C_l$ & so these would
ship identically \\
\varid{aeroDCm} & $\alpha\times\delta_a$ & $\Delta C_m$ & zero ---
an aircraft with no \\
\varid{aeroDCn} & $\alpha\times\delta_a$ & $\Delta C_n$ & roll control \\
\addlinespace
...
\paragraph{Rate derivatives past stall.}
\label{sec:ratetaper}
The derivatives are computed by finite difference on the attached solve
and have no meaning once the flow separates. Both available
treatments---holding the last attached value, or zeroing
discontinuously---are wrong; the model applies a linear taper
\begin{equation}
C_{(\cdot)}(\alpha) \;\longrightarrow\; C_{(\cdot)}(\alpha)\cdot
\max\!\left(0,\ \min\!\left(1,\ \frac{40\dg - \alpha}{20\dg}\right)\right),
\end{equation}
which is the conservative choice in the sense that it does not grant a
simulator attached-flow damping in deep stall. This is a declared
...
\emph{This block is specified but not yet generable.}

\begin{longtable}{@{}lllp{46mm}@{}}
\caption{Interaction tables (6). Blocked pending an upstream-induction
model.}
\label{tab:cpltables} \\
\toprule
\textbf{varID} & \textbf{Axes} & \textbf{Quantity} & \textbf{Notes} \\
...
artifact indistinguishable from a physical null result. A semi-infinite
vortex cylinder, which has a closed-form upstream field, is the
identified prerequisite, and the interaction tables are blocked on it.
The file's structure reserves their place so that adding them later does
...
\subsection{The aileron tables cannot be generated, and why that is worth
a section}
\label{sec:aileron}
...
\emph{different} path than the tables themselves.
...
\paragraph{What the fix has to be.} It is solver-side rather than
driver-side. A deflected strip must carry the flap in its \emph{section}
description---\varid{StripSection::Alpha0Deg} shifted by the
thin-airfoil flap increment for its hinge position and deflection, and
ideally a flap-shifted stall angle---so that the section model, onto
which the coupling drives the lattice's circulation, knows the flap is
down. Until then the aileron tables are declared blocked rather than
generated, which is the same discipline applied to the interaction
tables of Section~\ref{sec:coupling}.
...
\item \textbf{No interaction increments yet}
(Section~\ref{sec:coupling}), pending an upstream-induction model. Until
they exist, the airframe tables are power-off and the model
underpredicts the separation delay the fan provides in transition.
\item \textbf{No aileron tables} (Section~\ref{sec:aileron}). The
generating path cannot represent a hinged surface, so \varid{aeroDCX}
through \varid{aeroDCn} are declared blocked rather than shipped as
zeros. The model presently has no roll control input.
\item \textbf{Parasite drag is a conceptual-design estimate}
(Sections~\ref{sec:drag} and~\ref{sec:parasiteresults}), carrying the
conventional $\pm10$--$20\%$ accuracy of a component buildup. It is
moreover incomplete in one direction only: the duct's own separated drag
at incidence and any body-wake/wing interference are not modelled, and
both omissions push the same way, so \varid{aeroCD0} is a lower bound at
high incidence.
...
dedicated study.
\item \textbf{Rate derivatives are tapered, not computed, past
$\alpha = 20\dg$} (Section~\ref{sec:ratetaper}).
...
\begin{itemize}
\item \textbf{Reconcile the variable identifiers against S-119 Annex A}
(Section~\ref{sec:standards}), the standard variable names. Where a
standard name exists it should be adopted; where the namespacing
requirement genuinely conflicts, the departure should be recorded in the
file header rather than left implicit.
...
\item Extend the parasite estimate with the duct's separated drag at
incidence, the one contribution the slender-body form of
Eq.~\eqref{eq:cd0} cannot supply.
\item Build the upstream-induction model and unblock \varid{coupling*}.
\item \textbf{Unblock the aileron tables} (Section~\ref{sec:aileron}) by
carrying the flap into the strip's section description, so that a
deflection survives the viscous coupling. This is a behaviour change to
a tested solver module and wants its own branch.
