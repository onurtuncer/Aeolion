...
inline Aeolion::MeshIO::PartMesh BuildSyntheticWingMesh(double span, double rootChord, double tipChord,
    ...
    using Aeolion::Vlm::Vec3; using Aeolion::Vlm::Pi; using Aeolion::Vlm::Cross;
    Aeolion::MeshIO::PartMesh part;
    part.Name = name;
    double halfSpan = span / 2.0;
    double sweep = sweepDeg * Pi / 180.0;
    double dihedral = dihedralDeg * Pi / 180.0;
    ...
    auto twistOf = [&](double y) { return (twistTipDeg * Pi / 180.0) * (std::fabs(y) / halfSpan); };
